...
// function to get the plaform name given the platform id 
...
//function to get the Device name given the platform id and device id
...
// Fallback function to get error string from error code
...
// Helper function to check error codes returned by OpenCL calls
...
// Helper function to read kernel source code from file and add it to the sources vector
void AddSources(cl::Program::Sources& sources, const string& file_name) {
	...
	string* source_code = new string(
		(istreambuf_iterator<char>(file_stream)),
		istreambuf_iterator<char>()
	);
	sources.push_back(make_pair(source_code->c_str(), source_code->length() + 1));
}

// Helper function to list all platforms and devices, and return the info as a string
...
// Helper function to get profiling info for a given event and resolution, and return it as a string
string GetFullProfilingInfo(const cl::Event& evnt, ProfilingResolution resolution) {
	...
	switch (resolution) {
	case PROF_NS: sstream << " [ns]"; break; // default resolution
	case PROF_US: sstream << " [us]"; break; // microseconds
	case PROF_MS: sstream << " [ms]"; break; // milliseconds
	case PROF_S: sstream << " [s]"; break;  //	seconds
	default: break;
	}
